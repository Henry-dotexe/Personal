...
void eliminaprimero(NodoReparto*& lista);
int main() {
    FILE* f;
    pedido pedi;
...
void eliminaprimero(NodoReparto*& lista) {
    NodoReparto* aux = lista;
    lista = lista->next;
    delete(aux);
}
void insertapediord(NodoReparto* lista2, pedido pedi, char nom[30]) {
    NodoReparto* nuevo = new NodoReparto(), * aux = lista2, * ant = NULL;
    nuevo->info = pedi;
    ...
    if(lista2==NULL){//Por si la lista auxiliar aun no tiene nodos, se inserta de primero
        ...
        ant->next = nuevo; //insertamos el pedido nuevo entre ant y aux
        nuevo->next = aux;
    }
}
void mostrar(NodoReparto* primerReparto) {
...
void insertarOrdenado(Nodoarbol*& raiz, int dato){
    ...
    p = raiz;
    int j = 0;
    f = fopen("Comercios.dat", "rb+"); //Abro el archivo
